    ...
    for data in raw_data:
        for station in data['stations']:
            if station['id'] not in already_in:
                stations = pd.concat([stations, pd.DataFrame(station,
                                                            index=[0], 
                                                            columns=KEEP_COLS)], 
                                                            ignore_index=True)
...
def get_k_closest_stations(
    ...
    stations_copy['distance'] = stations_copy.apply(_calculate_distance, axis=1, args=(target_station,))
    stations_copy.sort_values(by='distance', inplace=True)
    return stations_copy.iloc[:k]


def _calculate_distance(row, target_station):
    return geodesic((row['latitude'], row['longitude']), (target_station['latitude'], target_station['longitude'])).km
